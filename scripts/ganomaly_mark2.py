...
class GANomaly_mark2(BaseModel):
    def __init__(self, config):
        """
        Args:
            config:
        """
        super(GANomaly_mark2, self).__init__(config)
        self.build_model()
        self.init_saver()
        ...
        with tf.variable_scope("Generator", custom_getter=getter, reuse=tf.AUTO_REUSE):
            with tf.variable_scope("Encoder_1"):
                ...
                with tf.variable_scope(net_name):
                    ...
                    # Current layer is  Batch_size x 14 x 14 x 128
                net_name = "Layer_2"
                with tf.variable_scope(net_name):
                    ...
                    # Current layer is  Batch_size x 7 x 7 x 256
                net_name = "Layer_3"
                with tf.variable_scope(net_name):
                ...
                with tf.variable_scope(net_name):
                    ...
                    # Current layer is  Batch_size x 1 x 1 x noise

            noise_gen = enc_layer_4

            with tf.variable_scope("Decoder"):
                gen_noise_entry = enc_layer_4
                net_name = "Layer_1"
                with tf.variable_scope(net_name):
                    ...
                    dec_layer_1 = tf.concat([enc_layer_3, dec_layer_1], axis=-1)
                ...
                with tf.variable_scope(net_name):
                    ...
                    dec_layer_2 = tf.concat([enc_layer_2, dec_layer_2], axis=-1)
                    ...
                    # Current layer is Batch_size x 7 x 7 x 512
                net_name = "Layer_3"
                with tf.variable_scope(net_name):
                    ...
                    dec_layer_3 = tf.concat([enc_layer_1, dec_layer_3], axis=-1)
                    ...
                    # Current layer is Batch_size x 14 x 14 x 256
                net_name = "Layer_4"
                with tf.variable_scope(net_name):
                    ...
                    dec_layer_4 = tf.nn.tanh(dec_layer_4)

                    # Current layer is Batch_size x 14 x 14 x 128
            image_rec = dec_layer_4
