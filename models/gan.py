...
class GAN(BaseModel):
    ...
    def build_model(self):
        ...
        with tf.variable_scope("Generator"):
            ...
            x_g = tf.keras.layers.Dense(7 * 7 * 256, activation="relu", use_bias=False,
            ...
            x_g = tf.keras.layers.Conv2DTranspose(128, (5, 5), strides=(1, 1), padding="same", use_bias=False,
                                                  kernel_initializer=tf.truncated_normal_initializer(stddev=0.02))(x_g)
            assert x_g.get_shape().as_list() == [None, 7, 7, 128]
            x_g = tf.keras.layers.BatchNormalization(momentum=self.config.batch_momentum)(x_g)
            x_g = tf.keras.layers.LeakyReLU(alpha=self.config.leakyReLU_alpha)(x_g)

            x_g = tf.keras.layers.Conv2DTranspose(128, (5, 5), strides=(2, 2), padding="same", use_bias=False,
                                                  kernel_initializer=tf.truncated_normal_initializer(stddev=0.02))(x_g)
            ...
            x_g = tf.keras.layers.Conv2DTranspose(1, (5, 5), strides=(1, 1), padding="same", use_bias=False,
