...
class ALAD_TF(BaseModel):
    ...
    def build_model(self):
        ...
        self.image_tensor = tf.placeholder(tf.float32, shape=[None] + self.config.trainer.image_dims, name="x")
        ...
        with tf.variable_scope("ALAD"):
            ...
            # Encoder
            self.z_gen = self.encoder(self.image_tensor,
            ...
            # Recreated Noise 
            self.rec_z = self.encoder(self.img_gen,
                                      do_spectral_norm=self.config.spectral_norm)

            # Discriminator_xz
            l_encoder, inter_layer_inp_xz = self.discriminator_xz(self.image_tensor, self.z_gen,
                                                                  do_spectral_norm=self.config.do_spectral_norm)
            l_generator, inter_layer_rct_xz = self.discriminator_xz(self.img_gen, self.noise_tensor,
                                                                    do_spectral_norm=self.config.spectral_norm)

            # Discriminator_xx
            x_logit_real, inter_layer_inp_xx = self.discriminator_xx(self.image_tensor, self.image_tensor,
                                                                     do_spectral_norm=self.config.spectral_norm)
            x_logit_fake, inter_layer_rct_xx = self.discriminator_xx(self.image_tensor, self.rec_img,
                                                                     do_spectral_norm=self.config.spectral_norm)

            # Discriminator_zz
            z_logit_real, _ = self.discriminator_zz(self.noise_tensor, self.noise_tensor,
                                                    do_spectral_norm=self.config.spectral_norm)
            z_logit_fake, _ = self.discriminator_zz(self.noise_tensor, self.rec_z,
                                                    do_spectral_norm=self.config.spectral_norm)
        ...
        with tf.name_scope("Loss_Functions"):
            ...
            self.loss_discriminator = self.dis_loss_xz + self.dis_loss_xx + self.dis_loss_zz if \
                self.config.trainer.allow_zz else self.dis_loss_xz + self.dis_loss_xx
        ...
        with tf.name_scope('Optimizers'):
            ...
            self.dxzvars = [v for v in all_variables if v.name.startswith("ALAD/Discriminator_Model_xz")]
            self.dxxvars = [v for v in all_variables if v.name.startswith("ALAD/Discriminator_Model_xx")]
            self.dzzvars = [v for v in all_variables if v.name.startswith("ALAD/Discriminator_Model_zz")]
            ...
            self.update_ops_gen = tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope="ALAD/Generator_Model")
            self.update_ops_enc = tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope="ALAD/Encoder_Model")
            self.update_ops_dis_xz = tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope="ALAD/Discriminator_Model_xz")
            self.update_ops_dis_xx = tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope="ALAD/Discriminator_Model_xx")
            self.update_ops_dis_zz = tf.get_collection(tf.GraphKeys.UPDATE_OPS, scope="ALAD/Discriminator_Model_zz")
            ...
            with tf.control_dependencies(self.update_ops_gen):
                self.gen_op = self.optimizer.minimize(self.loss_generator,
                                                      global_step=self.global_step_tensor, var_list=self.gvars)
            with tf.control_dependencies(self.update_ops_enc):
                self.enc_op = self.optimizer.minimize(self.loss_encoder,
                                                      global_step=self.global_step_tensor, var_list=self.evars)

            with tf.control_dependencies(self.update_ops_dis_xz):
                self.dis_op_xz = self.optimizer.minimize(self.dis_loss_xz,
                                                         global_step=self.global_step_tensor, var_list=self.dxzvars)

            with tf.control_dependencies(self.update_ops_dis_xx):
                self.dis_op_xx = self.optimizer.minimize(self.dis_loss_xx,
                                                         global_step=self.global_step_tensor, var_list=self.dxxvars)

            with tf.control_dependencies(self.update_ops_dis_zz):
                self.dis_op_zz = self.optimizer.minimize(self.dis_loss_zz,
                                                         global_step=self.global_step_tensor, var_list=self.dzzvars)
            ...
            self.train_dis_op_xz, self.xz_ema = train_op_with_ema_dependency(self.dxzvars,
                                                                             self.dis_op_xz)
            self.train_dis_op_xx, self.xx_ema = train_op_with_ema_dependency(self.dxxvars,
                                                                             self.dis_op_xx)
            self.train_dis_op_zz, self.zz_ema = train_op_with_ema_dependency(self.dzzvars,
                                                                             self.dis_op_zz)

        with tf.variable_scope("ALAD"):
            # with tf.variable_scope("Encoder_Model"):
            self.z_gen_ema = self.encoder(self.image_tensor, getter=sn.get_getter(self.enc_ema),
            ...
            l_encoder_emaxx, inter_layer_inp_emaxx = self.discriminator_xx(self.image_tensor, self.image_tensor,
                                                                           getter=sn.get_getter(self.xx_ema),
                                                                           do_spectral_norm=self.config.trainer.spectral_norm)
            l_generator_emaxx, inter_layer_rct_emaxx = self.discriminator_xx(self.image_tensor, self.rec_x_ema,
                                                                             getter=sn.get_getter(self.xx_ema),
                                                                             do_spectral_norm=self.config.trainer.spectral_norm)
        ...
        with tf.name_scope("Testing"):
            with tf.variable_scope("Scores"):
                ...
                score_fm = tf.norm(fm, ord=self.config.trainer.degree, axis=1,
                                   keep_dims=False, name='d_loss')
        ...
        if self.config.log.enable_summary:
            ...
            with tf.name_scope('summary'):
                ...
                selfsum_op_valid = tf.summary.merge_all('v')
    ...
    def encoder(self, img_tensor, getter=None, do_spectral_norm=True):
        ...
        with tf.variable_scope("Encoder_Model", reuse=tf.AUTO_REUSE, custom_getter=getter) as scope:
            img_tensor = tf.reshape(img_tensor, [-1, 28, 28, 1])
            net_name = "layer_1"
            with tf.variable_scope(net_name):
                net = layers.conv2d(img_tensor, filters=128, kernel_size=4, strides=2, padding="same",
                                    kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01), name="conv")
                net = tf.layers.batch_normalization(inputs=net, momentum=self.config.trainer.batch_momentum,
                                                    training=self.is_training, name="bn")
                net = tf.nn.leaky_relu(features=net, alpha=self.config.trainer.leakyReLU_alpha, name="leaky_relu")

            net_name = "layer_2"
            with tf.variable_scope(net_name):
                net = layers.conv2d(net, filters=256, kernel_size=4, strides=2, padding="same",
                                    kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01), name="conv")
                net = tf.layers.batch_normalization(inputs=net, momentum=self.config.trainer.batch_momentum,
                                                    training=self.is_training, name="bn")
                net = tf.nn.leaky_relu(features=net, alpha=self.config.trainer.leakyReLU_alpha, name="leaky_relu")
            ...
            net_name = "layer_3"
            with tf.variable_scope(net_name):
                net = layers.conv2d(net, filters=512, kernel_size=4, strides=2, padding="same",
                                    kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01), name="conv")
                net = tf.layers.batch_normalization(inputs=net, momentum=self.config.trainer.batch_momentum,
                                                    training=self.is_training, name="bn")
                net = tf.nn.leaky_relu(features=net, alpha=self.config.trainer.leakyReLU_alpha, name="leaky_relu")
            ...
            net_name = "layer_4"
            with tf.variable_scope(net_name):
                net = layers.conv2d(net, filters=self.config.trainer.noise_dim, kernel_size=4, strides=1,
                                    padding="valid",
                                    kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01), name="conv")
    ...
    def generator(self, noise_tensor, getter=None, ):
        ...
        Args:
            ...
            reuse: sharing variables or not
        """
        with tf.variable_scope("Generator_Model", reuse=tf.AUTO_REUSE, custom_getter=getter) as scope:
            net = tf.reshape(noise_tensor, [-1, self.config.trainer.noise_dim])
            net_name = "layer_1"
            with tf.variable_scope(net_name):
                net = tf.layers.dense(net, units=7 * 7 * 512,
                                      kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01),
                                      name="fc")
                net = tf.layers.batch_normalization(inputs=net, momentum=self.config.trainer.batch_momentum,
                                                    training=self.is_training, name="dense/bn")
                net = tf.nn.relu(features=net, name="dense/relu")
                net = tf.reshape(net, shape=[-1, 7, 7, 512])

            net_name = "layer_2"
            with tf.variable_scope(net_name):
                net = tf.layers.Conv2DTranspose(filters=512, kernel_size=5, strides=(1, 1), padding="same",
                                                kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01),
                                                name="tconv1")(net)
                net = tf.layers.batch_normalization(inputs=net, momentum=self.config.trainer.batch_momentum,
                                                    training=self.is_training, name="tconv1/bn")
                net = tf.nn.relu(features=net, name="tconv1/relu")

            net_name = "layer_3"
            with tf.variable_scope(net_name):
                net = tf.layers.Conv2DTranspose(filters=256, kernel_size=5, strides=(2, 2), padding="same",
                                                kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01),
                                                name="tconv2")(net)
                net = tf.layers.batch_normalization(inputs=net, momentum=self.config.trainer.batch_momentum,
                                                    training=self.is_training, name="tconv2/bn")
                net = tf.nn.relu(features=net, name="tconv2/relu")

            net_name = "layer_4"
            with tf.variable_scope(net_name):
                net = tf.layers.Conv2DTranspose(filters=128, kernel_size=5, strides=(2, 2), padding="same",
                                                kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01),
                                                name="tconv3")(net)
                net = tf.layers.batch_normalization(inputs=net, momentum=self.config.trainer.batch_momentum,
                                                    training=self.is_training, name="tconv3/bn")
                net = tf.nn.relu(features=net, name="tconv3/relu")

            net_name = "layer_5"
            with tf.variable_scope(net_name):
                net = tf.layers.Conv2DTranspose(filters=1, kernel_size=5, strides=(1, 1), padding="same",
                                                kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01),
                                                name="tconv4")(net)
    ...
    def discriminator_xz(self, img_tensor, noise_tensor, getter=None,
        ...
        """
        layers = sn if do_spectral_norm else tf.layers
        with tf.variable_scope("Discriminator_Model_xz", reuse=tf.AUTO_REUSE, custom_getter=getter):
            net_name = "x_layer_1"
            with tf.variable_scope(net_name):
                x = layers.conv2d(img_tensor, filters=128, kernel_size=4, strides=2, padding="same",
                                  kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01), name="conv1")
                x = tf.nn.leaky_relu(features=x, alpha=self.config.trainer.leakyReLU_alpha, name="conv1/leaky_relu")

            net_name = "x_layer_2"
            with tf.variable_scope(net_name):
                x = layers.conv2d(img_tensor, filters=256, kernel_size=4, strides=2, padding="same",
                                  kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01), name="conv2")
                x = tf.layers.batch_normalization(inputs=x, momentum=self.config.trainer.batch_momentum,
                                                  training=self.is_training, name="tconv2/bn")
                x = tf.nn.leaky_relu(features=x, alpha=self.config.trainer.leakyReLU_alpha, name="conv2/leaky_relu")
            net_name = "x_layer_3"
            with tf.variable_scope(net_name):
                x = layers.conv2d(img_tensor, filters=512, kernel_size=4, strides=2, padding="same",
                                  kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01), name="conv2")
                x = tf.layers.batch_normalization(inputs=x, momentum=self.config.trainer.batch_momentum,
                                                  training=self.is_training, name="tconv3/bn")
                x = tf.nn.leaky_relu(features=x, alpha=self.config.trainer.leakyReLU_alpha, name="conv3/leaky_relu")
            ...
            net_name = "y_layer_1"
            with tf.variable_scope(net_name):
                y = layers.conv2d(y, filters=1024, kernel_size=1, strides=1, padding="same",
                                  kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01), name="conv")
                y = tf.nn.leaky_relu(features=y, alpha=self.config.trainer.leakyReLU_alpha, name="leaky_relu")
                y = tf.layers.dropout(y, rate=self.config.trainer.dropout_rate, training=self.is_training,
                                      name="dropout")
    ...
    def discriminator_xx(self, img_tensor, recreated_img, getter=None,
                         do_spectral_norm=True):
        """ Discriminator architecture in tensorflow
        ...
        """
        layers = sn if do_spectral_norm else tf.layers
        with tf.variable_scope("Discriminator_Model_xx", reuse=tf.AUTO_REUSE, custom_getter=getter):
            net = tf.concat([img_tensor, recreated_img], axis=1)
            net_name = "layer_1"
            with tf.variable_scope(net_name):
                net = layers.conv2d(net, filters=64, kernel_size=5, strides=2, padding="same",
                                    kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01),
                                    name="conv1")
                net = tf.nn.leaky_relu(features=net, alpha=self.config.trainer.leakyReLU_alpha, name="conv2/leaky_relu")
                net = tf.layers.dropout(net, rate=self.config.trainer.dropout_rate, training=self.is_training,
                                        name="dropout")
            ...
            net_name = "layer_2"
            with tf.variable_scope(net_name):
                net = layers.conv2d(net, filters=128, kernel_size=5, strides=2, padding="same",
                                    kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01),
                                    name="conv2")
                net = tf.nn.leaky_relu(features=net, alpha=self.config.trainer.leakyReLU_alpha, name="conv2/leaky_relu")
                net = tf.layers.dropout(net, rate=self.config.trainer.dropout_rate, training=self.is_training,
            ...
            with tf.variable_scope(net_name):
                net = tf.layers.dense(net, units=1,
                                      kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01), name="fc")
                logits = tf.squeeze(net)

        return logits, intermediate_layer

    def discriminator_zz(self, noise_tensor, recreated_noise, getter=None,
                         do_spectral_norm=True):
        """ Discriminator architecture in tensorflow
        ...
        with tf.variable_scope("Discriminator_Model_zz", reuse=tf.AUTO_REUSE, custom_getter=getter):
            y = tf.concat([noise_tensor, recreated_noise], axis=-1)

            net_name = "y_layer_1"
            with tf.variable_scope(net_name):
                y = layers.dense(y, units=64, kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01),
                                 name="fc")
            ...
            net_name = "y_layer_2"
            with tf.variable_scope(net_name):
                y = layers.dense(y, units=32, kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01),
                                 name="fc")
                y = tf.nn.leaky_relu(features=y, alpha=self.config.trainer.leakyReLU_alpha)
                y = tf.layers.dropout(y, rate=self.config.trainer.dropout_rate, training=self.is_training,
                                      name="dropout")
            ...
            net_name = "y_layer_3"
            with tf.variable_scope(net_name):
                y = layers.dense(y, units=1, kernel_initializer=tf.random_normal_initializer(mean=0.0, stddev=0.01),
                                 name="fc")
                logits = tf.squeeze(y)
