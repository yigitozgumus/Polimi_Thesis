...
class GanKeras(BaseModelKeras):
    ...
    def build_model(self):
        ...
        inputs_d = tf.keras.layers.Input(shape=self.config.trainer.image_dims)
        x_d = Conv2D(
            32,
            (5, 5),
            strides=(2, 2),
        ...
        x_g = BatchNormalization(momentum=self.config.trainer.batch_momentum)(x_g)
        # f(x) = alpha * x for x < 0, f(x) = x for x >= 0.
        x_g = LeakyReLU(alpha=self.config.trainer.leakyReLU_alpha)(x_g)
        ...
        self.discriminator = Model(
            inputs=base_discriminator.inputs, outputs=base_discriminator.outputs
        )
        self.discriminator.compile(
            loss="binary_crossentropy", optimizer=self.optimizer, metrics=["accuracy"]
        )
        self.generator = Model(
            inputs=base_generator.inputs, outputs=base_generator.outputs
        )
        frozen_D = Model(
            inputs=base_discriminator.inputs, outputs=base_discriminator.outputs
        )
        ...
        z_input = Input(shape=[self.config.trainer.noise_dim])
