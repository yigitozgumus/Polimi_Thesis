...
class GAN_mark2(BaseModel):
    ...
    def build_model(self):
        ...
        with tf.name_scope("Discriminator_Loss"):
            self.disc_real_loss = tf.reduce_mean(
                tf.losses.sigmoid_cross_entropy(
                    multi_class_labels=tf.ones_like(real_output), logits=real_output
                )
            )
            self.disc_gen_loss = tf.reduce_mean(
                tf.losses.sigmoid_cross_entropy(
        ...
        tf.summary.image("Real_Image", tf.reshape(self.real_image, [-1, 28, 28, 1]))
        ...
        self.generator_vars = [
            v for v in all_variables if v.name.startswith("Generator")
        ]
        self.discriminator_vars = [
            v for v in all_variables if v.name.startswith("Discriminator")
        ]
        # Initialize and Connect the Optimizers
        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
        with tf.control_dependencies(update_ops):
            self.gen_optimizer = tf.train.AdamOptimizer(
                learning_rate=self.config.generator_l_rate,
                beta1=self.config.optimizer_adam_beta1,
            )
            self.disc_optimizer = tf.train.AdamOptimizer(
                learning_rate=self.config.discriminator_l_rate,
                beta1=self.config.optimizer_adam_beta1,
            )
